...
%      MARKERFCOLOR     -- default is 'b'
%      MARKEROCOLOR     -- default is [1 .5 0]
%      MARKERTCOLOR     -- default is 'r'
%      MARKERLINEWIDTH  -- default is 1
%      MARKERFONTSIZE   -- default is 10
...
for i=1:numel(ax)
    ...
    switch lower(userdata.function)
        ...
        case {'plot1' 'sft' 'ftan'}
            ...
            % modified by Fan Jan 7 2019
            % bug occurs in the 243th line because of fpos is single, need
            % to change it into double presicion
            fpos=double(fpos);
            
            % draw markers & flags
            hold(ax(i),'on');
            % modified by Fan Jan 7 2019
            % some of the marked time in userdata.markers.times is NAN, so
            % bug occurs here. In order to solve this bug in a easy way, I
            % just put the NAN time to 0
%             ind_nan = isnan(userdata.markers.times);
%             for ii = 1:13
%                 if (ind_nan(ii))
%                     userdata.markers.times(ii) = 0;
%                 end
%             end
            ...
            hmark{i}(4:13)=plot(ax(i),[userdata.markers.times(4:13); userdata.markers.times(4:13)],range,...
                ...
                'linewidth',opt.MARKERLINEWIDTH);
            for j=0:9
                % change the location of the notation
                if (j == 1)
                    hflag{i}(4+j)=text(userdata.markers.times(4+j),0.5*fpos,userdata.markers.names{4+j},...
                    'horizontalalignment',opt.MARKERHORZALIGN,...
                    'verticalalignment',opt.MARKERVERTALIGN,...
                    'color',opt.MARKERTCOLOR,...
                    'edgecolor',opt.MARKERTCOLOR,...
