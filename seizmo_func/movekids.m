...
if(isscalar(kids))
    ...
    % modified by Fan Jan 8 2019. cell2mat can not deal with the cell array
    % that contains cell component. so need to be fixed
    for ii = 1:1:length(kids)
        axx(ii) = get(kids(ii),'parent');            
    end
    for ii = 1:1:length(kids)-1
        for jj = ii+1:1:length(kids)
            kk = 1;
            p = axx(1);
            if (~isequal(axx(ii),axx(jj)))
                kk = kk+1;
                p(kk) = axx(jj);
            end
        end
